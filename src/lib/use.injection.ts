...
export type InjectorFunction<
  ...
  Providers extends ProvidersList = any
> = (this: InjectorThis<Env, Providers>, event: Event) => Promise<any>;
...
export type InjectionContext<T extends Record<string, any>> = EnvironmentVars<T> & {
 ...
 *
 * @param fn Must be written in standard format, this will be your handler function.
...
export const useInjection = <
  ...
  Providers extends ProvidersList = any
>(
  fn: InjectorFunction<Event<Data>, Env, Providers>,
  params: InjectorOptions
) =>
  async function (
    ...args: [InjectionContext<Env>, Event<Data>, ServerlessCallback]
  ): Promise<ReturnType<InjectorFunction<Event<Data>, Env, Providers>>> {
    ...
    const validateToken =
      _.isUndefined(params?.validateToken) || _.isNull(params?.validateToken)
        ? false
    ...
    const { request = {}, cookies = {}, Token, ...values } = event;
    ...
    try {
      if (validateToken) {
        ...
        const validation: any = await validator(Token, env.ACCOUNT_SID, env.AUTH_TOKEN);
      ...
      return callback(null, await fn.apply(that, [values as any]));
    } catch (err: any) {
